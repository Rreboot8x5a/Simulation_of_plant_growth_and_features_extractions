...
def visual_plant_sub(plant1,name='plant'):
	...
	if True:
		print('Il faut voir comment je vais faire l opposite decussate')
	print(str(nodes_cor))
	print(str(type(nodes_cor)))
	print("balise nodes_cor est au-dessus")
	print(str(nodes_cor.T[1]))
	orty = nodes_cor.T[1]
	coord = [nodes_cor.T[3], nodes_cor.T[4], nodes_cor.T[5]]
	yolo = nodes_cor.T[0]
	fig= go.Scatter3d(
		x=nodes_cor.T[3],
		y=nodes_cor.T[4],
	...
	# 	marker=dict(
	# 		size=3,
	# 		color=nodes_cor.T[0],  # nodes_cor.T[1] is organ type, nodes_cor.T[2] is the connection number of a node
...
#	plant.openXML("../../modelparameter/plant/" + name)
	# plant.openXML("modelparameter/plant/" + name)
	plant.openXML("/home/roberto/CplantBox/CPlantBox-master/" + name)
...
def create_arabidopsis():
	...
	s5 = pb.StemRandomParameter(rs)
	l1 = pb.LeafRandomParameter(rs)
	l2 = pb.LeafRandomParameter(rs)
	l4 = pb.LeafRandomParameter(rs)
...
# 	plant.openXML('../../modelparameter/plant/'+name)
	plant.openXML("/home/roberto/CplantBox/CPlantBox-master/" + name)
	# plant.readParameters("/home/roberto/CplantBox/CPlantBox-master/" + name)
	# print(str(plant.OrganTypes)) pas d'attributs organtypes pour plantbox.Plant
	print("c")
	plant = create_basil()
	# plant = create_arabidopsis()
	# plant = creacrea()
	# lavie = plant.getOrganRandomParameter(3)[1].rlt
	seeds = plant.getOrganRandomParameter(pb.OrganTypes.seed)
	print(seeds)
	roots = plant.getOrganRandomParameter(pb.OrganTypes.root)
	stems = plant.getOrganRandomParameter(pb.OrganTypes.stem)
	...
	plant.simulate(time)
	plant.write("{}.vtp".format(str(output)))
	test = plant.getOrgans()
	areuh = []
	for i in range(len(test)):
		if test[i].organType() == 4:
			if test[i].getParameter('subType') == 3:
				areuh.append(1)
	polyl = plant.getPolylines()
	polystem = plant.getPolylines(3)
	polyleaf = plant.getPolylines(4)
	caca = pb.SegmentAnalyser(plant)
	return plant;
...
def python_nodes(plant):
	...
	print("en-dessous c'est les organtype dans une liste")
	print(str(organtype))
	print("yolo")
	lines = list([list(s) for s in plant.getPolylines()])
	# print("getPolylines")
	# print(str(plant.getPolylines()))
	# print("fin polylines")
	# poly = plant.getPolylines()
	# organ = plant.getOrgans()
	# organ2 = plant.getOrgans(4)
	# param = organ2[1].getParameter('length')
	# param2 = organ2[1].getParameter('type')
